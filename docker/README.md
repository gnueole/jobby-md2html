...
├── prod/
│   └── docker-compose.yml     # Remote Production Orchestration
...
* **`docker/dev/docker-compose.yml`**: Mounts the local project directory as a volume (`../../:/app`) for live-reloading during code development. Binds services strictly to `127.0.0.1` for local safety.
* **`docker/prod/docker-compose.yml`**: Deploys the production stack using pre-built images from the GitHub Container Registry. Incorporates Traefik for public SSL termination (`80`/`443`) and resource boundaries for stability.
