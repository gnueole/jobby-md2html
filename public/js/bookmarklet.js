...
export function compileBookmarkletCode(url, token) {
    ...
    return `javascript:(function(){const token="${token}";const webhookUrl="${url}";const editorOrigin="${editorOrigin}";const getTxt=(s)=>document.querySelector(s)?.innerText?.trim()||"";let description=getTxt('#job-details')||getTxt('.jobs-description')||getTxt('.jobs-box__html-content')||"";if(description.length<50){description=window.getSelection().toString();}const title=encodeURIComponent(getTxt('.job-details-jobs-unified-top-card__job-title')||document.title);const company=encodeURIComponent(getTxt('.job-details-jobs-unified-top-card__company-name')||"");const jobUrl=encodeURIComponent(window.location.href);const shortDesc=encodeURIComponent(description.substring(0,1800));const helperUrl=\`\${editorOrigin}/bookmarklet-helper.html?webhookUrl=\${encodeURIComponent(webhookUrl)}&token=\${encodeURIComponent(token)}&job_title=\${title}&company=\${company}&job_url=\${jobUrl}&job_description=\${shortDesc}\`;window.open(helperUrl,'_blank');})();`;
}
